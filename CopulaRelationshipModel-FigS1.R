...
## Steve Bellan, 2014
####################################################################################################
## Note that age at first intercourse is given in years in the DHS (though those that were at the
## time of marriage may be in months since marital date was given as a month).
####################################################################################################
## NOTE: We used the Uganda 2004-2005 AIS when generating the Ugandan copula model. This survey was
## restricted and we have not provided it along with the rest of the data. Refitting with only the
...
## source('CopulaRelationshipModel-FigS1.R')
...
file.nm <- ifelse(weight, "Uganda weighted marginal fits.pdf", "Uganda marginal fits.pdf") # pdf names
pdf(file.path(outdir,file.nm), w = 14, h = 10)
ug <- which(ds.nm=='Uganda')
ii <- ug
...
sigmas <- abind(tempcwt$cov, along = 3)
...
cophist <- function(country) {
    ...
    psdat <- add.vars(rcop(country, browse=F, N = 10^5, tint = 2011)) ## call rcop to simulate pseudopopulation of 10^5 couples
    for(i.typ in 1:2) {                                     ## for real & fit
        for(i.transf in 1) { ## for untransformed & log-transformed
            typ <- typs[i.typ]
            transf <- transfs[i.transf]
            if(!file.exists(file.path(outdir,transf))) dir.create(file.path(outdir,transf)) # create directory
            jpeg(file.path(outdir, paste0('Figure S1 - ', ds.nm[country], typ, ' ',transf, ".jpg")), w = 1200, h = 1200)
