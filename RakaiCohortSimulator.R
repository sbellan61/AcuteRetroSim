...
niter <- 8*1000
nburn <- 2*1000
dpars <- c(acute.sc = 26, late.sc = 10, bp = .007, dur.ac = 2, dur.lt = 9, dur.aids = 10)
ldpars <- log(dpars)
...
if(simul) { ## simulate data
    ...
    if(excl.by.err) wtab.do <- wtab.rl else wtab.do <- wtab.rl.no.err
...
edpars
sigma <- cov.wt(posts[,parnames])$cov ## posterior covariance matrix, then plot what proposal distr is gonna look like
...
hollsbpairs(expfposts[,tracenames], truepars = edpars[tracenames], show.lines = T, ## plot posterior correlations after fit phase
        ...
        cex = 1, col = "black", nrpoints = 100, do.jpeg = T, browse=F)
apply(expfposts[tracenames], 2, function(x) signif(quantile(x, c(.025, .5, .975)),3))
edpars[tracenames]
...
mlposts <- posts[posts$nll < max(posts$nll) + qchisq(.95,6)/2,]

apply(mlposts, range)
    
holl.wrp(4, force.inits = ldpars, jit = 1,
         sd.props = sd.props,  wtab = wtab.do,
         multiv = T, covar = sigma, 
         verbose = T, verbose2 = F, tell = 100, 
         niter = niter, nthin = 1, nburn = nburn, browse=F)
